...
const DEFAULT_W = {winrate:0.35,general:0.35,goals:0.30,viteza:0.0,tehnica:0.0,strategie:0.0,aparare:0.0};
let W = {...DEFAULT_W};
...
function computeSmartRatingComponents(p, context = {}){
    ...
    const { bonus: tagBonus, signals: tagSignals } = computeTagBonus(activeTags);
    const afterTags = afterActivity + tagBonus;
    ...
    const final = parseFloat(Math.max(1, Math.min(10, afterTags + synergyBonus)).toFixed(2));
...
const PRESETS = {
    balanced:    {winrate:.35,general:.35,goals:.30,viteza:0,tehnica:0,strategie:0,aparare:0},
    defensive:   {winrate:.30,general:.50,goals:.20,viteza:0,tehnica:0,strategie:0,aparare:0},
    performance: {winrate:.35,general:.25,goals:.40,viteza:0,tehnica:0,strategie:0,aparare:0}
};
...
function updateWeightsSum(){
    const sum = ['winrate','general','goals'].reduce((s,k)=>s+(W[k]||0),0);
...
async function saveAlgorithm(){
    const mainKeys = ['winrate','general','goals'];
    const sum = mainKeys.reduce((s,k)=>s+(W[k]||0),0);
    if (Math.abs(sum-1) > 0.02) {
        showToast(`⚠️ Suma ponderi (Win Rate + Rating + Goluri) = ${(sum*100).toFixed(0)}% — trebuie să fie 100%!`);
        return;
    }
    try{
...
function buildModalStats(p){
    ...
    let formulaHtml = `<strong>Smart Rating</strong>: WinRate×${(W.winrate*100).toFixed(0)}% + Goluri×${((W.goals||0)*100).toFixed(0)}% + Rating×${wTot.toFixed(0)}%`;
    ...
    rc.parts.forEach(part => {
        ...
        steps.push({icon:part.icon, label:part.label, val:null, note, color:col(part.delta), delta:part.delta});
    });
    ...
    if(activeTags2.length>0){
        ...
        const tagDetails = rc.tagSignals
            .filter(s=>s.dir!==0)
            .map(s=>`${s.tag.emoji}${s.raw>=0?'+':''}${s.raw.toFixed(2)}`)
            .join(' ');
        const note = tagDetails + (tagDetails?' → ':'') + `net ${rc.tagBonus>=0?'+':''}${rc.tagBonus.toFixed(2)}`;
        steps.push({icon:'🏷️', label:`Tag-uri (${posCount>0?'+'+posCount+' pos':''}${negCount>0?' −'+negCount+' neg':''}${activeTags2.length-posCount-negCount>0?' ~neu':''})`, val:null, note, color:col(rc.tagBonus), delta:rc.tagBonus});
    }
...
function previewSmartRating(p, snap){
    ...
    // Win Rate și Goluri rămân calculate din statisticile reale ale jucătorului,
    // exact ca în computeSmartRatingComponents — ca "Preview" și "Real" să
    // difere DOAR prin ce se schimbă efectiv la sliders (notele pe categorii).
    const winrateScore = 5 + (getWinrateShrunk(p) - 0.5) * 10;
    const goalsScore   = getGoalsScoreRelative(p);

    const parts = [
        { score:winrateScore,        w:W.winrate||0 },
        { score:goalsScore,          w:W.goals||0 },
        { score:catScores.general,   w:W.general||0 },
        { score:catScores.viteza,    w:W.viteza||0 },
        { score:catScores.tehnica,   w:W.tehnica||0 },
        { score:catScores.strategie, w:W.strategie||0 },
        { score:catScores.aparare,   w:W.aparare||0 },
    ];
    const wSum = parts.reduce((s,c)=>s+c.w, 0);
    ...
    const wG = W.general||0;
    const contribs = {
        general:   parseFloat((wSum>0 ? (catScores.general-5)*wG/wSum : 0).toFixed(2)),
        viteza:    parseFloat((wSum>0 ? (catScores.viteza-5)*(W.viteza||0)/wSum : 0).toFixed(2)),
        tehnica:   parseFloat((wSum>0 ? (catScores.tehnica-5)*(W.tehnica||0)/wSum : 0).toFixed(2)),
        strategie: parseFloat((wSum>0 ? (catScores.strategie-5)*(W.strategie||0)/wSum : 0).toFixed(2)),
        aparare:   parseFloat((wSum>0 ? (catScores.aparare-5)*(W.aparare||0)/wSum : 0).toFixed(2)),
    };

    return {
        final: parseFloat(Math.max(1,Math.min(10,base+tagBonus)).toFixed(2)),
        contribs, penalized, tagBonus, signals
    };
}
